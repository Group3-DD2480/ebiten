...
func init() {
	...
	ebitenCursor = ebiten.CreateCursor(&ebitenImage, 0, 0)
}
...
func main() {
	...
	g := &Game{
		grids: map[image.Rectangle]*glfw.Cursor{
			image.Rect(100, 100, 200, 200): ebitenCursor,
			image.Rect(200, 100, 300, 200): ebitenCursor,
			image.Rect(300, 100, 400, 200): ebitenCursor,
			image.Rect(100, 200, 200, 300): ebitenCursor,
			image.Rect(200, 200, 300, 300): ebitenCursor,
			image.Rect(300, 200, 400, 300): ebitenCursor,
		},
		gridColors: map[image.Rectangle]color.Color{},
	}
	for rect, _ := range g.grids {
